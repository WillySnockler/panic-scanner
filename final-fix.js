...
button{cursor:pointer!important;pointer-events:auto!important;touch-action:manipulation!important}
...
function enhance(){
 ...
 if(s&&!s.dataset.psEnhanced){s.dataset.psEnhanced='1';s.classList.add('psTerminal');const i=$('q');if(i){i.placeholder='Search ticker or company  ·  NVDA, MU, MOWI';const m=document.createElement('div');m.className='psMeta';m.innerHTML='<i class="psDot"></i><span>LIVE SECURITIES</span><span>·</span><span>Stocks · ETFs · Companies</span>';s.insertAdjacentElement('afterend',m);const q=document.createElement('div');q.className='psQuick';['NVDA','MU','AAPL','TSLA','MSFT','MOWI'].forEach(x=>{const b=document.createElement('button');b.type='button';b.textContent=x;b.addEventListener('click',()=>{i.value=x;window.searchStocks()});q.appendChild(b)});m.insertAdjacentElement('afterend',q)}}
 const bar=document.querySelector('.investigateBar');
 if(bar){const b=bar.querySelector('b');if(b)b.textContent='Deep Search';const p=bar.querySelector('span');if(p)p.textContent='Fundamentals · catalysts · news · technical risk';const btn=bar.querySelector('button');if(btn&&!btn.dataset.psDeepFixed){btn.dataset.psDeepFixed='1';btn.textContent='Run Deep Search →';btn.type='button';btn.removeAttribute('onclick');btn.addEventListener('click',function(e){e.preventDefault();e.stopPropagation();window.__psManualDeepSearch=true;const fn=window.investigate;if(typeof fn==='function')return fn.call(this,e)},true)}}
 wireAllButtons();
}
