...
<div class="psModal" id="psSettingsModal"><div class="psModalShell"><div class="psModalHead"><div><b>Settings</b><span>Preferences are separate from account plans.</span></div><button class="psClose" onclick="psClose('psSettingsModal')">×</button></div><div class="psModalBody"><div class="psSettingsGrid">
  ...
  const script = `<script id="ps-production-fix-js">
function psOpen(id){document.getElementById(id)?.classList.add('open')}
function psClose(id){document.getElementById(id)?.classList.remove('open')}
window.openDrawer=function(){psOpen('psAccountModal')}
window.closeDrawer=function(){psClose('psAccountModal');psClose('psSettingsModal')}
document.addEventListener('keydown',e=>{if(e.key==='Escape')closeDrawer()})
document.querySelectorAll('.psModal').forEach(m=>m.addEventListener('click',e=>{if(e.target===m)m.classList.remove('open')}))
</script>`;

  html = html.replace('</style>', css + '</style>');
  html = html.replace('<div class="search">', '<div class="heroProof"><div><b>REAL PRICE ACTION</b><span>Inspect the move, not a generic chart.</span></div><div><b>NEWS + EVIDENCE</b><span>Connect the reaction to what changed.</span></div><div><b>RESEARCH WORKFLOW</b><span>Finish with a structured conclusion.</span></div></div><div class="search">');
  html = html.replace('MARKET OVERREACTION RESEARCH', 'MARKET INTELLIGENCE · REAL RESEARCH');
  html = html.replace('Find fear.<br><span>Challenge the panic.</span>', 'See the move.<br><span>Challenge the panic.</span>');
  html = html.replace('Scan a company, see the real price action, follow the news that moved it, then investigate whether the market reaction looks excessive.', 'Scan a company, inspect the real price action, connect it to the news and evidence, then investigate whether the market reaction looks excessive.');
  html = html.replace('Search a stock — BlackRock or BLK', 'Search a stock or company — BlackRock or BLK');
  ...
  res.setHeader('Vercel-Cache-Tag', 'panic-scanner-home');
