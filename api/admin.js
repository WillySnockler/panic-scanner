...
module.exports = async (req, res) => {
  ...
  try {
    ...
    if (req.method === 'GET') {
      const [users, promos] = await Promise.all([
        db('profiles?select=id,email,display_name,plan,is_admin,subscription_status,subscription_current_period_end,vip_until&order=created_at.desc&limit=500'),
    ...
    if (req.method === 'POST') {
      const body = req.body || {}, code = String(body.code || '').trim().toUpperCase();
      ...
      const created = await db('promo_codes', { method: 'POST', headers: { Prefer: 'return=representation' }, body: JSON.stringify({ code, plan, discount_percent: percent, active: true, max_redemptions: body.max_redemptions == null ? null : Math.max(1, Number(body.max_redemptions)), redemption_count: 0, expires_at: body.expires_at || null }) });
      return res.status(201).json({ ok: true, promo: created?.[0] || null });
    }
    const body = req.body || {}, target = String(body.user_id || '').trim();
  ...
  } catch (e) { console.error('admin api error', e); return res.status(500).json({ error: e.message || 'Admin request failed.' }); }
};
