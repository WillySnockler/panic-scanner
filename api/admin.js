  ...
  try {
    ...
    if (req.method === 'GET') {
      const [users, promos] = await Promise.all([
        db('profiles?select=id,email,plan,is_admin,is_vip,subscription_status,subscription_current_period_end&order=created_at.desc&limit=500'),
        db('promo_codes?select=code,plan,percent_off,active,max_redemptions,redemptions,expires_at,created_at&order=created_at.desc&limit=500')
      ]);
    ...
    if (req.method === 'POST') {
      ...
      const plan = ['Standard', 'Pro', 'Elite'].includes(body.plan) ? body.plan : 'Pro';
      const percent = Math.max(0, Math.min(100, Number(body.percent_off || 0)));
      if (!/^[A-Z0-9_-]{3,64}$/.test(code)) return res.status(400).json({ error: 'Invalid promo code.' });
      const created = await db('promo_codes', {
        method: 'POST',
        headers: { Prefer: 'return=representation' },
        body: JSON.stringify({ code, plan, percent_off: percent, active: true, max_redemptions: body.max_redemptions == null ? null : Math.max(1, Number(body.max_redemptions)), expires_at: body.expires_at || null })
      });
    ...
    const patch = {};
    if (body.plan && ['Standard', 'Pro', 'Elite'].includes(body.plan)) patch.plan = body.plan;
    ...
    await db(`profiles?id=eq.${encodeURIComponent(target)}`, { method: 'PATCH', headers: { Prefer: 'return=minimal' }, body: JSON.stringify(patch) });
