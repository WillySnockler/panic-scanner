...
async function updateProfile(userId, patch) {
  if (!userId) return;
  // Upsert is deliberate: Stripe webhooks can legitimately arrive before any
  // application-side profile row exists. Never silently lose an entitlement.
  const r = await supabaseRequest('profiles', {
    method: 'POST',
    headers: { Prefer: 'resolution=merge-duplicates,return=minimal' },
    body: JSON.stringify({ id: userId, ...patch })
  });
  if (!r.ok) throw new Error(`Supabase profile sync failed: ${r.status}`);
}
...
module.exports = async (req, res) => {
  ...
  catch (e) { console.error('stripe webhook body read error', e); return res.status(400).json({ error: 'Unable to read webhook body.' }); }
  ...
  try {
    ...
    if (event.type === 'customer.subscription.deleted' && userId) {
      await updateProfile(userId, {
        plan: 'Standard', subscription_status: 'canceled',
        stripe_subscription_id: null, stripe_price_id: null, subscription_current_period_end: null
      });
    }
