...
function closeAi(){const p=$('psPanel');if(p){p.classList.remove('open');p.style.removeProperty('display');p.setAttribute('aria-hidden','true');}}
function openAi(){const p=$('psPanel');if(p){p.style.removeProperty('display');p.classList.add('open');p.setAttribute('aria-hidden','false');}}
...
function wireClose(){const p=$('psPanel');if(!p)return;const b=p.querySelector('.psClose');if(!b||b.dataset.safeWired)return;b.dataset.safeWired='1';b.type='button';b.addEventListener('click',e=>{e.preventDefault();e.stopPropagation();closeAi();});b.addEventListener('pointerup',e=>{e.preventDefault();e.stopPropagation();closeAi();});}
...
const originalAi=window.psOpenAi;
if(typeof originalAi==='function'){window.psOpenAi=function(){openAi();return originalAi.apply(this,arguments);};}
...
function run(){hideCreate();wireClose();wireOpen();manualDeep();enhanceSearch();}
run();
