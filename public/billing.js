...
(function(){
  ...
  async function syncToken(session){
    if(session?.access_token){localStorage.setItem('pswAccessToken',session.access_token);if(session.refresh_token)localStorage.setItem('pswRefreshToken',session.refresh_token)}
    else{localStorage.removeItem('pswAccessToken');localStorage.removeItem('pswRefreshToken')}
  }
  async function init(){
    const {data:{session}}=await client.auth.getSession();
    await syncToken(session);
    ...
    window.fetch=async function(input,init){
      ...
      if(/^\/api\/(investigate|account|billing)(\?|$)/.test(url)){
        const headers=new Headers(init?.headers||{});
        if(!headers.has('Authorization')){const {data:{session}}=await client.auth.getSession();if(session?.access_token)headers.set('Authorization','Bearer '+session.access_token)}
        init={...(init||{}),headers};
      }
      if(url.startsWith('/api/investigate?')&&/[?&]mode=research(?:&|$)/.test(url)){
        const plan=effectivePlan();
        const depth=plan==='elite'?'elite':plan==='pro'?'pro':'standard';
        input=url.replace(/([?&])depth=(elite|pro|standard)/,'$1depth='+depth);
      } else if(url.startsWith('/api/investigate?')&&/[?&]symbol=/.test(url)&&/[?&]depth=elite/.test(url)){
        const plan=effectivePlan();
        input=url.replace(/([?&])depth=elite/,'$1depth='+(plan==='elite'?'elite':plan==='pro'?'pro':'standard'));
      }
  ...
  async function createAccount(){
    const email=document.getElementById('email')?.value.trim(),password=document.getElementById('password')?.value||'';
    if(!email||password.length<8){toast('Use an email and a password of at least 8 characters.');return}
  ...
  async function refreshProfile(){
    ...
    else profile=profile||{plan:'standard',subscription_status:'free'};
    if(profile?.is_admin===true) profile.plan='elite';
    const plan=effectivePlan();
    localStorage.setItem('psEmail',user.email||'');localStorage.setItem('psPlan',plan);localStorage.setItem('psAdmin',profile?.is_admin===true?'1':'0');
    const badge=document.querySelector('.online');if(badge)badge.textContent=profile?.is_admin===true?'● OWNER / ELITE':isVip()?'● VIP / ELITE':`● ${plan.toUpperCase()} ACCOUNT`;
    patchButtons();
  }
  ...
  async function startCheckout(plan,interval='monthly'){
    ...
    const r=await fetch('/api/billing',{method:'POST',headers:{'Content-Type':'application/json',...headers},body:JSON.stringify({action:'checkout',plan,interval})});const d=await r.json();if(!r.ok){toast(d.error||'Checkout failed.');return}window.location.href=d.url;
  }
  ...
  function patchButtons(){
    ...
    document.querySelectorAll('.plan').forEach(card=>{
      ...
      if(title==='Pro'||title==='Elite'){
        card.dataset.annual='1';const b=document.createElement('button');b.className='outline';b.style='width:100%;margin-top:7px';b.textContent=title==='Pro'?'Annual · 1,499 NOK':'Annual · 2,990 NOK';b.onclick=()=>startCheckout(title.toLowerCase(),'yearly');card.appendChild(b);
      }
    });
  ...
  window.investigate=function(){
    ...
    if(p==='standard' && !profile){openModal('accountModal');toast('Sign in to use your daily deep investigation.');return}
    return originalInvestigate?.();
  };
  const originalDrawElite=window.drawElite;
  window.drawElite=function(){if(effectivePlan()==='elite')return originalDrawElite?.()};
  load();
})();
